import * as React from "react"
import { SVGProps } from "react"

const EyeSvg = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 80.794 80.794"
    // style={{
    //   enableBackground: "new 0 0 80.794 80.794",
    // }}
    xmlSpace="preserve"
    {...props}
  >
    <g fill="#9b9b9b">
      <path d="M79.351 38.549c-.706-.903-17.529-22.119-38.953-22.119-21.426 0-38.249 21.216-38.955 22.119L0 40.396l1.443 1.847c.706.903 17.529 22.12 38.955 22.12 21.424 0 38.247-21.217 38.953-22.12l1.443-1.847-1.443-1.847zM40.398 58.364c-15.068 0-28.22-13.046-32.643-17.967 4.425-4.922 17.576-17.966 32.643-17.966 15.066 0 28.218 13.045 32.642 17.966-4.426 4.922-17.577 17.967-32.642 17.967z" />
      <path d="M40.397 23.983c-9.052 0-16.416 7.363-16.416 16.414 0 9.053 7.364 16.417 16.416 16.417s16.416-7.364 16.416-16.417c0-9.051-7.364-16.414-16.416-16.414zm0 26.83c-5.744 0-10.416-4.673-10.416-10.417 0-5.742 4.672-10.414 10.416-10.414 5.743 0 10.416 4.672 10.416 10.414 0 5.744-4.673 10.417-10.416 10.417z" />
    </g>
  </svg>
)

export default EyeSvg
